...
| Tipografía | Inter · Space Grotesk — fuentes locales autoalojadas (sin dependencia CDN) |
| Seguridad | PBKDF2-SHA256 (600k iter.) · HMAC-SHA256 · SubtleCrypto API |
| Código | ~6 425 líneas JS · ~5 976 líneas CSS · ~2 587 líneas HTML |
...
|----------|----------------------|
| `Content-Security-Policy` | `default-src 'self'; script-src 'self'; style-src 'self'; font-src 'self'; img-src 'self' data: blob:; connect-src 'self'; worker-src 'self'; manifest-src 'self'; frame-ancestors 'none';` — sin dominios externos; fuentes servidas localmente |
...
```
SplashLab/
├── Aquatech.html       # App shell — estructura, plantillas y overlays (~2 587 líneas)
├── Aquatech.css        # Estilos — diseño responsivo mobile-first (~5 976 líneas)
├── Aquatech.js         # Lógica completa (~6 425 líneas)
├── sw.js               # Service Worker (cache offline, versión brazada-v12)
├── manifest.json       # Manifiesto PWA (nombre: Aquatech)
├── vercel.json         # Cabeceras de seguridad HTTP para Vercel
├── fonts/              # Fuentes autoalojadas: Inter + Space Grotesk (woff2)
